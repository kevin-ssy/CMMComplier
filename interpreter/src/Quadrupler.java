...
public class Quadrupler {
	...
	public Quadrupler(Node treeRoot) {
		...
		postOrder(treeRoot);
		JFrame jf = new JFrame("intermediate code");
		jf.setSize(400, 200);
		Container container = jf.getContentPane();
		String ic = new String();
		for (Quadruple q : quads) {
			String temp = new String();
			temp = "[op: " + q.oper + " vars:" + q.var1 + " " + q.var2
					+ " result:" + q.result + "]";
			ic += temp + "\n";
	...
	public void postOrder(Node nod) {
		...
		if (childnum == 0) {
			...
			if (childnum == 1) {
				...
				// 0 child stores variable name, 1 stores the value that will be
				// assigned
				...
				// <while> --> beforeexp <exp> jumpfalse <statement> jump
					...
					// System.err.println("the jump Node:" + jump.method); //
					// test
